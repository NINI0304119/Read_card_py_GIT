...
class MainForm(Form):
	...
	def __init__(self, owner):
		...
		if test_flag == 1:
			self.test_btn = Button(self)
			self.test_btn.SetProps(Parent=self, Caption="測試按鈕", OnClick=self.test_btnClick)
			self.test_btn.SetBounds(button_left, self.scan_file.top + label_height + height_space , button_weight, button_height)

			self.test_label = Label(self)
			self.test_label.SetProps(Parent=self,Caption = test_label_text)
			self.test_label.SetBounds(label_right, self.scan_file_state.top + label_height + height_space, label_weight, label_height)
	...
	if test_flag == 1:
		def test_btnClick(self, Sender):
			#Application.MessageBox("index", 'this is title', MB_ICONINFORMATION);
			self.test_label.Caption = test_label_text + "qwe"
